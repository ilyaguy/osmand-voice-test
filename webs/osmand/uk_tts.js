	...
	switch (metricConst) {
		case "km-m":
			if (dist < 17) {
				...
				return (tts ? Math.round((dist/10.0)*10).toString() : ogg_dist((dist/10.0)*10)) + " " + dictionary["metriv"];
			} else if (dist < 1000) {
		...
		case "mi-y":
			if (dist < 17) {
				...
				return (tts ? Math.round((2*dist/100.0/0.9144)*50).toString() : ogg_dist((2*dist/100.0/0.9144)*50)) + " " + dictionary["yardov"]; 
			} else if (dist < 2414) {
				return dictionary["around_1_mile"];
			} else {
				return (tts ? Math.round(dist/1609.3).toString() : ogg_dist(dist/1609.3)) + " " + plural_mi(dist/1609.3);
